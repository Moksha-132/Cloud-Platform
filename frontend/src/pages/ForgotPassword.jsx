...
import { Mail, ArrowRight, ArrowLeft, Loader2, ShieldCheck, Zap } from 'lucide-react';
  ...
  return (
    <div className="base bg-[#F8FAFC]">
      <nav className="p-8 flex justify-between items-center max-w-7xl mx-auto w-full">
        ...
        <Link to="/login" className="bg-white hover:bg-slate-50 text-slate-600 border border-slate-100 py-3 px-8 rounded-full font-bold text-sm flex items-center gap-2 transition-all shadow-sm">
      ...
      <main className="flex-1 flex items-center justify-center px-6 py-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-12 md:p-16 rounded-[4rem] shadow-2xl border border-slate-100 max-w-xl w-full text-center">
          <div className="mb-10 flex flex-col items-center">
            ...
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">Forgot Password?</h2>
          ...
          {message && <div className="bg-emerald-50 text-emerald-600 p-5 rounded-2xl text-sm font-bold mb-8 border border-emerald-100">{message}</div>}
          ...
          <form className="space-y-8" onSubmit={handleSubmit}>
            <div className="space-y-3 text-left">
              ...
              <div className="relative">
                ...
                <input type="email" required placeholder="Enter your email" className="w-full bg-[#F8FAFC] border border-slate-100 rounded-2xl py-6 pl-16 pr-6 font-bold outline-none focus:bg-white focus:ring-4 focus:ring-amber-500/10 transition-all" value={email} onChange={(e) => setEmail(e.target.value)} />
              </div>
            </div>
